...
let protocolJsonCorrectEntryType: any = {
  protocol: {
    metadata: {
      completionDate: new Date(),
      creator: "David Massanek",
    ...
    entries: [
      ...
      },
      {
        ...
        type: "select",
        choices: ["feld1"],
        possibleChoices: ["feld1", "feld2", "feld3"],
        config: {
          minChoices: 1,
          maxChoices: 1,
        },
...
it("Test ProtocolParser retruns missing receipt date", () => {
  let parsed: ParserError | Protocol = new ProtocolParserV1(
    protocolJsonMissingReceiptDate
  ).parse();
  let parseError: ParserError = new ParserError([]);
  if (parsed instanceof ParserError) {
    parseError = parsed;
...
it("Test ProtocolParser retruns missing parser version", () => {
  let parsed: ParserError | Protocol = new ProtocolParserV1(
    protocolJsonMissingParserVersion
  ).parse();
  let parseError: ParserError = new ParserError([]);
  if (parsed instanceof ParserError) {
    parseError = parsed;
...
it("Test ProtocolParser returns missing comletion Date", () => {
  let parsed: ParserError | Protocol = new ProtocolParserV1(
    protocolJsonMissingCompletionDate
  ).parse();
  let parseError: ParserError = new ParserError([]);
  if (parsed instanceof ParserError) {
    parseError = parsed;
...
it("Test ProtocolParser returns missing label", () => {
  let parsed: ParserError | Protocol = new ProtocolParserV1(
    protocolJsonMissingLabel
  ).parse();
  let parseError: ParserError = new ParserError([]);
  if (parsed instanceof ParserError) {
    parseError = parsed;
    expect(
      parseError.messages.indexOf("Missing field 'label' in protocol.metadata")
    ).not.toBe(-1);
  } else return false;
});

it("Test correct EntryType NUMBER", () => {
  let parsed: ParserError | Protocol = new ProtocolParserV1(
    protocolJsonCorrectEntryType
  ).parse();
  let protocol: Protocol = new Protocol();
  if (parsed instanceof Protocol) {
    protocol = parsed;
    //TODO vorerst Number und Select, deshalb 2. Später auch table, also 3
    expect(protocol.entries.length).toBe(2);
